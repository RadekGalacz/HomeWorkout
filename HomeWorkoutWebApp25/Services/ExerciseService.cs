...
public class ExerciseService {
    ...
    public async Task<IEnumerable<ExerciseDto>> GetAllAsync() {
        var allExercises = await _dbContext.Exercises.OrderByDescending(exer => exer.BodyPartId).ToListAsync();
        var exerciseDtos = new List<ExerciseDto>();

        foreach (var exercise in allExercises) {
            var exerciseDto = ModelToDto(exercise);
    ...
    // Vrátí cvik podle ID
    internal async Task<ExerciseDto> GetByIdAsync(int id) {
